...
| current 2026-08-23 slice | approved five-shard critical path, routing audit, local audience/effects, formal archive contract, portable assets, four-limb IK/foot lock, editable skeleton mapping, constrained seven-action Agent boundary, navmesh/Rapier, large autosave and bundle split | local tests and browser smokes pass; branch/CI publication state must be read from Git rather than inferred from this document |
...
| Ruby Scene Builder | `215/215 PASS`, `build PASS`, rig-editor smoke `PASS` | portable asset store/package with legacy source-lock preservation, controlled character state machine, four-limb/two-hand IK with stationary foot locks, persistent skeleton mapping, exact seven-action Agent boundary, blocked-endpoint navmesh coverage, Rapier, audience runtime/effects and existing combined runtime |
