...
| CP03 Five Actions / Agent-Native Encounter | free input, five DSH roles, hash approval, governed 3D effects | `MERGED_LOCAL_SCRIPTED_VERIFIED + REMOTE_SYNC_VERIFIED + RUBY_FIVE_EFFECT_UI_ENGINEERING_VERIFIED + GEMINI_37_ADAPTER_STAGE_A_ENGINEERING_READY_ARCHIVED + BAKEOFF_ELIGIBLE_PREFLIGHT_ARCHIVED + THREE_FAILED_LIVE_RUNS`; the combined tree passes council, exact viewer approval, Ruby interaction/rollback, audience UI, five Three.js effects, rig, physics, persistence and archive-contract gates. The fixed-input Stage B machinery and anonymous comparison surface are locally scripted, and one fresh zero-call preflight is eligible; no paid bake-off, provider-quality finding, author selection or formal CP03 capture has occurred. | obtain fresh run-ID-bound approval for the one paid Stage B run, collect blinded human decisions, then complete Stage C representative 3D encounter and formal five-class archive |
...
| B6. Fixed-input Stage B bake-off machinery | `ELIGIBLE_PREFLIGHT_ARCHIVED / REAL_RUN_NOT_AUTHORIZED` | five frozen candidates, two repetitions, exact 28 planned / 30 maximum scheduling, provider-scoped retry validation, true DSH scripted transport, ten-class technical verifier, cryptographically blinded five-decision packet and reviewed 1280×720 / 26.2-second local capture. Task 7 run `cp03-model-bakeoff-20260824T055034Z` is `28 eligible / 0 excluded / 0 sent`, finds both named Keychain items by presence only, estimates USD 0.35545088 worst case and records provider requests 0; Key values remain unread and author selections remain 0. |
...
**Status:** `ELIGIBLE_PREFLIGHT_ARCHIVED / REAL_RUN_NOT_AUTHORIZED`
...
- Freshly authorised Task 7 created exactly one zero-call preflight, [`cp03-model-bakeoff-20260824T055034Z`](../checkpoints/cp03/model-bakeoff/cp03-model-bakeoff-20260824T055034Z/README.md). It archives current official price extracts and hashes, fixed role caps, and only the two named Keychain service/account references. Its result is `ELIGIBLE_AWAITING_EXPLICIT_APPROVAL`, `28 eligible / 0 excluded / 0 sent`, `providerRequestsMade: 0`, with both Keychain items present. The conservative official-rate estimate is USD `0.3190784` planned and `0.35545088` including one bounded retry reserve per provider.

Task 7 is complete at the preflight boundary only. It did not read either Key value, call a provider, create an approval record, run Stage B, or alter `pending-bakeoff`. The exact run ID and hashes now require a fresh explicit paid-run approval; no prior Live Run budget or generic “continue” authorises that boundary.
...
6. Preserve Task 7 run `cp03-model-bakeoff-20260824T055034Z` as the single eligible zero-call preflight: official price extracts and hashes frozen, both named Keychain items present, `28 eligible / 0 excluded / 0 sent`, and no Key value read or provider request.
7. Require a fresh approval bound to that exact run ID, hashes, five models, role caps, `28 planned / 30 maximum`, provider-scoped retry contract, USD `0.50` hard ceiling and forbidden capabilities before the one paid Stage B run. No generic continuation, earlier budget or Live Run approval carries over.
