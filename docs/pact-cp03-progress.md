...
| CP03 Five Actions / Agent-Native Encounter | free input, five DSH roles, hash approval, governed 3D effects | `MERGED_LOCAL_SCRIPTED_VERIFIED + REMOTE_SYNC_VERIFIED + RUBY_FIVE_EFFECT_UI_ENGINEERING_VERIFIED + GEMINI_37_ADAPTER_STAGE_A_ENGINEERING_READY_ARCHIVED + STAGE_B_PARTIAL_FAILED_ARCHIVED + STAGE_B_REPLACEMENT_REPAIR_LOCAL_ZERO_CALL_ARCHIVED + THREE_FAILED_LIVE_RUNS`; the combined tree passes council, exact viewer approval, Ruby interaction/rollback, audience UI, five Three.js effects, rig, physics, persistence and archive-contract gates. The one approved fixed-input Stage B run is preserved as `PARTIAL_FAILED_ARCHIVED / NO_TECHNICALLY_ELIGIBLE_PAIRS`: `24 sent / 2 accepted / 22 failed / 4 skipped`, technical `FAIL`, and `0 / 12` eligible pairs. A separate local repair passes 16/16 adversarial checks and has deterministic diagnostic media, but it created no replacement preflight or run and does not make the failed run a partial success. No author selection, routing replacement or formal CP03 capture has occurred. | resolve the remaining provider-compatibility risk, then obtain separate approval for a new preflight/run identity before any replacement provider call; only a technically eligible archive can proceed to human selection and the Stage C 3D encounter |
...
| `eafe2d5`–`94ca2d1` | Stage B diagnosis, author-approved replacement repair, comprehensive zero-call gate and local repair archive | model-facing role content is bound deterministically to runtime authority; references fail closed before durability; Conductor caps, terminal evidence, one-shot attempts, legacy identity and pair-local replacement evidence pass 16/16 local checks; the PNG/WebM packet is explicitly provider-unverified and created no preflight or replacement run |
| current 2026-08-24 combined slice | remote council, Gemini 3.7 adapter-readiness and fixed-input bakeoff chain plus Ruby's reach-constrained, batched four-limb IK/foot lock with smooth preview budgets, editable skeleton mapping and constrained seven-action Agent boundary | Ruby commits `e00cbc2` and `c1247e2` are already ancestors of remote tip `e27a693` and local repair tip `94ca2d1`; a fresh fetch found the remote tip to be the local branch's merge base, with local/remote exclusive counts `11 / 0`, so no new merge was required or manufactured. The current macOS combined gate passes Shared Contracts 48/48, Agent Host 492/492 plus typecheck/build, Scene Builder 238/238 plus build, and the five-action audience smoke |
...
| B6. Fixed-input Stage B bake-off machinery | `PARTIAL_FAILED_ARCHIVED / NO_TECHNICALLY_ELIGIBLE_PAIRS / ROUTING_PENDING` | run `cp03-model-bakeoff-20260824T055034Z` consumed its one exact approval and ended `24 sent / 2 accepted / 22 failed / 4 skipped`, with no retries and estimated cost USD `0.1464567` (`ESTIMATE_NOT_BILLING`). Its legacy result labelled 21 attempts as schema failures, plus one late timeout and four dependency skips; the deeper terminal diagnosis is recorded below. Archive/approval/plan/secret checks pass, but the full technical report fails and exposes `0 / 12` selectable pairs; final 1280×720 / 26.36-second media honestly shows zero candidates and no author decision. Production routing remains `pending-bakeoff`. |
...
- Production routing remains `pending-bakeoff`. One paid fixed-input Stage B attempt has now run but failed technical eligibility; provider/model quality, blinded human selection, an approved final routing manifest, Stage C representative 3D interaction, the formal CP03 checkpoint, deployment and public release are still not established.

### Fixed-input Stage B machinery and failed real run — 2026-08-24
...
- Task 7 created exactly one zero-call preflight, [`cp03-model-bakeoff-20260824T055034Z`](../checkpoints/cp03/model-bakeoff/cp03-model-bakeoff-20260824T055034Z/README.md). At that historical pre-run boundary its result was `ELIGIBLE_AWAITING_EXPLICIT_APPROVAL`, `28 eligible / 0 excluded / 0 sent`, `providerRequestsMade: 0`, with current official price extracts and hashes, fixed role caps, and both named Keychain items present by existence check only. Its conservative official-rate estimate was USD `0.3190784` planned and `0.35545088` including one bounded retry reserve per provider.
- The author then explicitly approved Task 8 for that exact run/preflight, USD `0.50`, one run and no automatic rerun/model substitution. Ruby's intervening scene-builder work was incorporated by strict fast-forward to combined commit `7e441d4`; it did not touch bake-off bindings. The zero-call gate returned `AUTHORIZED` before either Key value was loaded.
- The single real command produced the failed archive state `PARTIAL_FAILED_ARCHIVED / NO_TECHNICALLY_ELIGIBLE_PAIRS`: `24` provider requests, `2` accepted, `22` failed and `4` skipped; retry slots remained unused and the estimated—not billed—cost is USD `0.1464567`. The legacy result labelled 21 attempts `schema_failure`, one DeepSeek Guardian attempt crossed the 12-second boundary, and all four ConductorCommit cases were skipped after their intent dependency failed. Only both `deepseek-v4-flash / Archivist` repetitions were accepted; those two accepted outputs do not constitute a partial success because the technical archive is `FAIL` and every pair is ineligible.
- Joining the 24 attempt records to their DSH diagnostics shows that the 21 legacy schema labels collapse three different terminal classes: four Gemini 3.7 request/configuration HTTP 400 failures before any tool call (`streamCount=1`, `toolCallCount=0`); six output-cap stops without a tool call (`max-tokens`, `streamCount=1`, `toolCallCount=0`); and eleven malformed tool calls followed by an error/second stream (`streamCount=2`, `toolCallCount=1`). The separate twenty-second failure is the one hard timeout.
- The root cause is therefore a host/model authority and transport-design defect, not one uniform model-JSON defect. The old tool required agents to author the creative role content and the host-owned canonical runtime envelope together; it exposed schema/version, role/kind, session/turn identities, hashes, registry/routing versions and deadline identity as model clerical work. The transport also flattened terminal provider/max-token states into schema failure, applied the Conductor commit cap to intent, and allowed a rejected tool result to cause an undeclared second stream. Finally, canonical shape acceptance happened before exact registry-reference validation: the two accepted Archivist outputs cited an unregistered rights ID and still could not be technically eligible.
- The full verifier preserves the run as `FAIL`: archive, approval, exact plan and secret scan pass; exact completion/dispatch, contracts, grounding, timing, usage/cost and DSH trace do not. The partial-run base gate therefore makes every one of the 12 role/model pairs technically ineligible. The blind packet has zero candidates, no model is selected, and Task 9 cannot start from this evidence.
...
The local commits implement that conclusion without rewriting model content:
`eafe2d5` and `86ad65d` establish the diagnosis baseline and fail-closed
reference behavior; `fb90a9e` adds closed agent-owned submission contracts;
`3417318` and `94c9e08` make production and bake-off share the deterministic
binder, turn binding and exact reference validator; `cedcb26` freezes
phase-specific request policy; `d24807a` preserves terminal classes and
one-shot semantics; `9f9e4bc` separates global run status from replacement
pair-local eligibility while preserving the legacy archive; `c6b7b6d` adds
the 16-check zero-call gate; and `94ca2d1` archives the honestly labelled local
repair media and report. None of those commits proves provider compatibility.
...
### Verification snapshot — 2026-08-24 current macOS combined revalidation

The fetch-time live graph was linear rather than divergent: `HEAD` was
`94ca2d1decfa9f9fb06e8819aac971ed33d81a27`, remote truth was
`e27a693dfbf0359dc75e85922b33f002042136d7`, the merge base was the same remote
SHA, and `git rev-list --left-right --count HEAD...origin/...` returned
`11 0`. Remote-only commits were therefore zero and no synthetic two-parent
merge was created.

| Layer | Result | Scope |
|---|---|---|
| Shared CP03 contracts | `48/48 PASS` across 1 file | full current contract suite |
| PACT DSH host typecheck | `PASS` | `tsc -p tsconfig.json --noEmit`; no diagnostics |
| PACT DSH host build | `PASS` | `tsc -p tsconfig.build.json` |
| PACT DSH host tests | `492/492 PASS` across 40 files | full current host suite, including legacy archive and local repair gates |
| Ruby Scene Builder build | `PASS` | Vite transformed 179 modules |
| Ruby Scene Builder tests | `238/238 PASS` across 34 files | full current Scene Builder suite including Ruby IK, rigging and performance work |
| Browser audience smoke | `PASS` | all five actions; 5 proposals; 5 receipts; 7 screenshots; `providerRequestsMade: 0`; `checkpointEligible: false` |
| Provider / Keychain activity | `NOT RUN` | no provider call, paid run, Keychain value read, preflight or model substitution was authorised or performed |

### Historical verification snapshot — 2026-08-24 combined Windows integration
...
| Ruby Scene Builder | `build PASS`, `238/238 PASS` across 34 files | combined remote CP03/bake-off UI and local IK/retargeting/Agent behaviour tree; measured after the real merge and Task 8 archive capture |
...
That earlier Windows integration retained local IK hardening tip `e00cbc2` and remote
...
3. Keep Ruby's `e00cbc2` and `c1247e2` plus the council/adapter/bake-off history as the shared baseline. The 2026-08-24 live fetch found remote `e27a693` already behind local `94ca2d1` with zero remote-only commits; verify remote truth again before any later push and never replace either author's history with a force push.
