...
| current 2026-08-23 slice | approved five-shard critical path, routing audit, local audience/effects, formal archive contract, portable assets, reach-constrained and batched four-limb IK/foot lock with smooth preview budgets, editable skeleton mapping, constrained seven-action Agent boundary, navmesh/Rapier, large autosave and bundle split | local tests, general UI, rig-editor and camera-editor browser smokes pass; the private CP02 Case Pack is not present for a fresh CP02 browser rerun; branch/CI publication state must be read from Git rather than inferred from this document |
...
| Ruby Scene Builder | `225/225 PASS`, `build PASS`, general UI / rig-editor / camera-editor smoke `PASS` | portable asset store/package with legacy source-lock preservation, controlled character state machine, reach-constrained batched four-limb/two-hand IK with stationary foot locks and smooth preview budgets, persistent skeleton mapping, exact seven-action Agent boundary, blocked-endpoint navmesh coverage, Rapier, audience runtime/effects and existing combined runtime |
