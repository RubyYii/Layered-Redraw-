...
7. **Asset runtime** — a local OBJ or self-contained GLB can replace a selected placeholder without non-uniformly distorting the source. OBJ provides static geometry. GLB can additionally provide skins, bones, animation clips, morph targets, four limb-IK chains, gaze control, foot locks, and a target for imported animation-GLB retargeting. Every limb target is constrained to a world-scale reachable annulus with a small straight-chain singularity margin; zero-length and non-finite chains are rejected without writing invalid rotations. Per-limb diagnostics expose requested/effective targets, reach limits, residual error, clamp reason, and actual iterations. A 25-slot searchable bone-map editor persists explicit bindings (including intentional unmapping), reports confidence/missing/duplicate slots, and tests pose, both hands, and both feet without changing model bytes. `AnimationMixer` crossfades between semantic states; a phase-aware performance profile adds bounded two-hand contact planning, foot-lock/gaze policy and deterministic semantic Morph cues while preserving explicit expression overrides. The model and optional animation source are persisted by SHA-256 rather than remaining only in memory.
...
`buildReliefMeshData` resamples colour, alpha, and near-white relative depth onto an aspect-preserving grid. Faces crossing a configured depth discontinuity or transparent cutout are omitted. The OBJ serializer writes vertex colour, UV, normal, and face indices in one static file. The rig step maps 22 named joints onto sampled surface points, lets the user drag their image-space anchors, computes four normalized nearest-segment weights per vertex, and exports a self-contained skinned GLB. Bone names match the stable semantic map so generated characters immediately expose torso, arm, leg, hand, foot, and toe slots.

This is deliberately a 2.5D authoring accelerator. It does not infer metric scale, body semantics, occluded backs, thickness, watertight topology, subject segmentation, pose landmarks, joint limits, balance, fingers, or production skin weights. Transparent full-body front views are the intended input for the humanoid preset; arbitrary scenes are better kept as static OBJ reliefs. The portable binding preserves the exact processed RGB, depth preview, model, and rig recipe needed to inspect or reproduce the generated asset on another machine.
...
Actions, expressions, bones, hands, and feet accept either a semantic slot or the original GLB name. Retargeting accepts an animation GLB only when it contains a source skinned mesh and clips; incompatible names fail without replacing existing clips. `assetReport` exposes geometry/skin/bone counts, clips, morphs, mapping diagnostics, `twoHandIk`/`footLock`/`lookIk`/`fullBodyIk`, retarget capability, warnings, and current state-machine data. A future behavior tree or model provider should use the seven-action boundary and inspect its receipt rather than writing transforms or model URLs directly.
...
- Animation retargeting still lacks general humanoid rest-pose normalization. The runtime's “full-body IK” capability means two valid non-degenerate arm chains, two valid non-degenerate leg chains, distributed gaze, and stationary foot locks are all mapped. Reach-annulus clamping prevents numerical overextension but is not an authored joint-limit system, general balance solver, finger IK, gait-aware footstep planner, or terrain adaptation layer.
...
- Single-image OBJ/GLB generation is a relative-depth relief workflow. The editable 22-bone preset is useful for front-facing cutouts, but it is not pose estimation, subject segmentation, hidden-surface reconstruction, volumetric modelling, or production skinning.
