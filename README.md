...
v1.10 把单图骨架升级为通用拓扑编辑器：除正面人形外，提供侧面四足、正面鸟类和蛇形预设，也可新增、删除、镜像、改名、重设父级与拖动任意骨骼。每个关节可声明语义角色、运动链、末端执行器、前后深度和球形／铰链／扭转限位；运行时从 rig 配方生成 `walk / fly / bite / peck / wag / slither` 等能力清单，并用 `setAssetChainIk` 控制任意有效骨链。自定义拓扑、限位、能力与模型一同进入带哈希的跨机器工程包；这仍是用户校准的 2.5D 蒙皮工作流，不是自动动物姿态识别、肌肉仿真或生产级权重绘制。

v1.11 增加“多视角 → 可旋转灰模”工作台：正面加左右任一侧面即可在浏览器本地生成确定性的中性灰 visual hull，背面、另一侧、顶部和底部可继续收紧体积。每张照片都显示可调整的轮廓遮罩；OBJ、规范视角、阈值/方向/体素配方和不含图像内容的本地操作序列一起进入 SHA-256 可移植工程。它是供绘画者检查空间假设的灰模，不是摄影测量：不恢复纹理、米制尺度、相机标定、隐藏凹陷或生产拓扑。

v1.12 把深度估计接入多视角灰模，但保留纯轮廓为可比较基线：每个视角可在浏览器本机估算 Depth Anything V2 相对深度，切换灰度预览、反转近／远或停用该证据。相邻按钮分别生成纯轮廓基线与深度辅助条件；证据面板报告每个视角实际拒绝的体素和多视角共同拒绝量，不把作用计数冒充置信度。轮廓仍是硬外壳；深度只沿对应相机射线以可调影响和容差收紧可见表面。系统保存原始深度 PNG、启用／反转状态、模型元数据、基线／削减计数与操作轨迹，刷新后可复现。各图只做轮廓内稳健归一化，不冒充跨视角米制深度。
...
- 通用生物骨架：人形、四足、鸟类、蛇形与自定义骨架共享可编辑父子拓扑、镜像、关节限位、语义运动链、末端执行器和通用链式 IK；rig profile 随可移植工程保存。
- 多视角空间参考：规范视角照片在本地变成可检查、可修正、可旋转的 visual-hull 灰模；可选相对深度只收紧轮廓外壳，并能逐视角预览、反转或拒绝。来源角色、最终设置、深度工件与操作序列随工程保存，便于研究绘画者如何校正和信任算法空间参考。
...
同一位置的“多视角 → 可旋转灰模”不需要 Codex。为同一静止物体导入正面和一个侧面，逐张确认粉色轮廓，即可生成纯轮廓中性灰 OBJ；可选背面和顶部会继续约束体积。若需要更有用的可见表面，可对当前或全部视角运行浏览器内 Depth Anything V2，检查白近黑远的预览，再逐张反转、停用或调低影响。首次运行只下载模型权重，照片不上传。请保持直立、居中、相近焦距和干净背景；结果适合旋转观察与绘画构图，不应当作高精度扫描。
