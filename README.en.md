...
The editor imports OBJ or a self-contained GLB for a selected carrier without distorting proportions. GLB additionally exposes skins, bones, clips, morphs, semantic actions/expressions, hand-chain capability, and an animation-GLB retarget interface. Contact phases drive real world-space two-bone CCD hand IK. Model, animation, and RGB-D bytes persist in a content-addressed browser store and are included by **Save portable project package**. Radius-inflated static geometry produces a triangulated navigation corridor; the kinematic ownership layer still rejects illegal handoff/release and constrains held props; unconstrained `dynamic` bodies are stepped by Rapier gravity and contacts. Remaining limits include full humanoid rest-pose normalization, full-body/finger/foot IK, Recast-style dynamic navigation, and a game-grade rigid-body character controller.
