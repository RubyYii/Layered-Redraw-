...
v1.5 hardens the 3D runtime and adds the local CP03 audience slice. Large projects now recover through IndexedDB; OBJ/GLB, animation GLB, and RGB-D bytes enter a SHA-256 content store and can travel in a verified `.blockout.zip`. Skinned GLBs support compatible animation retargeting and contact-driven two-bone hand IK; approach planning uses a triangulated navigation mesh; and unconstrained dynamic bodies use a lazily loaded Rapier WASM gravity/contact world. `?case=pact-cp03` provides five governed actions, hash approval, Capability Gate execution, five transient Three.js effects, and Ruby receipts in a zero-call audience UI. The Agent Host also implements the five-role parallel critical path, 6/8 dispatch budget, routing audit, and a fail-closed five-class checkpoint archive contract. These local implementations are not a passing Gemini/DeepSeek Live Run or a completed CP03 archive.
...
- An experimental 3D director with continuous scenes, fixed-60Hz preview, deterministic 30fps rendering, hierarchical characters, curved motion, triangulated navigation, object interaction anchors, legal ownership transitions, collision proxies, lazy Rapier dynamic bodies, and an auditable agent-intent contract; it includes the complete 166-second *Window That Wasn't There* fixture.
...
- A painting-to-3D bridge that lets Scene Builder import `spatial-bridge.json` from a project folder, verify the RGB/depth-preview hashes and relative-scale contract, and mount a persistent but still non-metric textured height field through an editable carrier.
- Portable 3D projects that recover large JSON through IndexedDB and package content-addressed models, retarget animation sources, and RGB-D triplets beside the project in a hash-verified `.blockout.zip`.
- A local CP03 audience console with five distinct Three.js actions, immutable draft-hash approval/rejection, Capability Gate evidence, and runtime receipts, visibly labelled as a scripted zero-call harness rather than provider or checkpoint proof.
...
The editor imports OBJ or a self-contained GLB for a selected carrier without distorting proportions. GLB additionally exposes skins, bones, clips, morphs, semantic actions/expressions, a 25-slot persistent bone map, four limb CCD chains, gaze control, stationary foot locks, and an animation-GLB retarget interface. Model, animation, and RGB-D bytes persist in a content-addressed browser store and are included by **Save portable project package**. Radius-inflated static geometry produces a triangulated navigation corridor; the kinematic ownership layer still rejects illegal handoff/release and constrains held props; unconstrained `dynamic` bodies are stepped by Rapier gravity and contacts. Remaining limits include general humanoid rest-pose normalization, authored joint limits and balance, finger solving, gait-aware footstep planning, Recast-style dynamic navigation, and a game-grade rigid-body character controller.
